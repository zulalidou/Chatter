...
class RandomChat extends React.Component {
    ...
    async getLoggedInUsers() {
        let response = await fetch(`/get-logged-in-users?userID=${this.state.user1ID}`, {credentials: "include"})
    ...
    async getRoomID() {
        let response = await fetch(`/get-user-field-info?userID=${this.state.user1ID}&attribute=currentRoomOpen`, {credentials: "include"})
    ...
    async isRandomRoom(roomID) {
        console.log(roomID)
        const response = await fetch(`/is-random-room?roomID=${roomID}`, {credentials: "include"})
    ...
    async getUser2Data(roomID) {
        const response = await fetch(`/get-user2-data?user1ID=${this.state.user1ID}&roomID=${roomID}`, {credentials: "include"})
